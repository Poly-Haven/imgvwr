...
| Histogram (`F2`) | Distribution of the *displayed* values (after exposure / view transform), as additive R/G/B areas — or one white area when a channel is isolated (alpha included). The 1px spike at the right edge counts samples past the displayable maximum — drop the exposure to bring them back into range. `L` / `Sq` / `Log` pick the vertical scale; the eye button measures only the visible region, so you can zoom in for a focused reading |
| Levels handles (`F2`) | The two triangles under the histogram set the display black and white points, stretching that slice of the range back out to full. They share the graph's x axis, so you set them by reading the data. Drag the bar between them to slide both at once. Double-click to reset; `Ctrl+R` resets them along with the other adjustments |
